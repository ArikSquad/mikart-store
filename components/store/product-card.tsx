...
function PriceDisplay({ product, className = "" }: { product: StoreProduct; className?: string }) {
  const onSale = Boolean(product.totalPrice && product.totalPrice > product.totalPrice);

  return (
    <div className={className}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-baseline gap-2">
          <p className="font-black">{formatMoney(product.totalPrice, product.currency)}</p>
          {onSale ? (
            <span className="text-sm font-black text-[#9fa4b3] line-through">
              {formatMoney(product.totalPrice!, product.currency)}
            </span>
          ) : null}
        </div>
        {onSale ? (
